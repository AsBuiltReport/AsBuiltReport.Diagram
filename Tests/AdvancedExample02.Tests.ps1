...
Describe AdvancedExample02 {
    BeforeAll {
        ...
        $RunFile = & $ProjectRoot\AsBuiltReport.Diagram\Examples\AdvancedExample02.ps1 @PassParamsDot
    }
    ...
    Context 'Format Parameter Tests' {
        ...
        It 'Should exist Example1.png' {
            (& $ProjectRoot\AsBuiltReport.Diagram\Examples\Example15.ps1 @PassParamsPng).FullName | Should -Exist
        }
        It 'Should return error about unsupported Format' {
            { & $ProjectRoot\AsBuiltReport.Diagram\Examples\Example15.ps1 @PassParamsTif } | Should -Throw -ExpectedMessage "Cannot validate argument on parameter 'Format'. The argument `"tif`" does not belong to the set `"pdf,svg,png,dot,base64,jpg`" specified by the ValidateSet attribute. Supply an argument that is in the set and then try the command again."
        }
    }
    Context 'Graphviz Dot Node Tests' {
        Context 'Graphviz Dot Main Label Tests' {
            It 'Should match HTML label with embedded image' {
                ...
                $ExpectedText = "label=<<TABLE border='0' cellborder='0' cellspacing='20' cellpadding='10'><TR><TD ALIGN='center' colspan='1'><img src='AsBuiltReport.png'/></TD></TR><TR><TD ALIGN='center'><FONT FACE='Segoe Ui' Color='#565656' POINT-SIZE='24'>Web Application Diagram</FONT></TD></TR></TABLE>>,"

                $DotContent | Should -Match '>Web Application Diagram</FONT>'
        ...
        Context 'Graphviz Dot Node Icon (Label) Tests' {
            It 'Should match HTML label Info-Box node with embedded image' {
                ...
                $DotContent | Should -Match 'Info-Box'
                $DotContent | Should -Match '<B>This is a test text box.<BR/>It supports multiple lines of text.<BR/>You can customize the font, size, color, and background.</B>'
                $DotContent | Should -Match 'BGCOLOR="#FFFF00"'
                $DotContent | Should -Match 'BORDER="2"'
                $DotContent | Should -Match 'COLOR="#FF0000"'
                $DotContent | Should -Match 'POINT-SIZE="30"'
