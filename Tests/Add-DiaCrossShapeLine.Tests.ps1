BeforeAll {
    ...
    . (Join-Path -Path $PrivateFolder -ChildPath 'Add-DiaCrossShapeLine.ps1')
}

Describe Add-DiaCrossShapeLine {
    BeforeAll {
        [string]$DotOutPut = Add-DiaCrossShapeLine
        [string]$DotOutPutDebug = Add-DiaCrossShapeLine -DraftMode $true
        [string]$DotOutPutWithParams = Add-DiaCrossShapeLine -CrossShapeStart 'Start' -CrossShapeEnd 'End' -CrossShapeMiddle 'Middle' -CrossShapeMiddleTop 'Top' -CrossShapeMiddleDown 'Down'
        [string]$DotOutPutWithParamsArrowsTest = Add-DiaCrossShapeLine -Arrowtail box -Arrowhead diamond
        [string]$DotOutPutWithParamsLineTest = Add-DiaCrossShapeLine -LineStyle solid -LineWidth 3 -LineColor red
    ...
    It "Should return a error: Cannot validate argument on parameter 'Arrowtail'" {
        $scriptBlock = { Add-DiaCrossShapeLine @DotOutPutWithParamsArrowsTestError }
    ...
    It "Should return a error: Cannot validate argument on parameter 'LineWidth'" {
        $scriptBlock = { Add-DiaCrossShapeLine @DotOutPutWithParamsLineTestError }
        $scriptBlock | Should -Throw
    }
