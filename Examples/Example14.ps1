#    ** This time, we'll demonstrate the use of Add-NodeShape cmdlet to add custom shapes to the diagram (Part of AsBuiltReport.Diagram module). **
...
$example14 = & {
    ...
    SubGraph 3tier -Attributes @{Label = '3 Tier Concept'; fontsize = 22; penwidth = 1.5; labelloc = 't'; style = 'dashed,rounded'; color = 'darkgray' } {
        ...
        SubGraph Signature -Attributes @{Label = $Signature; fontsize = 22; penwidth = 1.5; labelloc = 'b'; labeljust = 'right'; style = 'invis'; color = 'darkgray' } {
            ...
            Add-NodeEdge -From 'WAN' -To 'Firewall' -HeadLabel 'port1' -EdgeColor 'black' -EdgeLabelFontSize 18 -EdgeLabelFontColor 'black' -EdgeLength 2 -Arrowhead 'normal' -Arrowtail 'normal' -LabelDistance 5 -EdgeThickness 3 -EdgeStyle 'dashed'
...
New-AbrDiagram -InputObject $example14 -OutputFolderPath $OutputFolderPath -Format $Format -MainDiagramLabel $MainGraphLabel -Filename Example14 -LogoName 'Main_Logo' -Direction top-to-bottom -IconPath $IconPath -ImagesObj $Images -DraftMode:$DraftMode