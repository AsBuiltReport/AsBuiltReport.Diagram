function New-AbrDiagram {
    ...
    begin {
        ...
        if ($Signature -and (([string]::IsNullOrEmpty($AuthorName)) -or ([string]::IsNullOrEmpty($CompanyName)))) {
            throw 'New-AbrDiagram: AuthorName and CompanyName must be defined if the Signature option is specified'
        }
        ...
        if ($Logo -and (-not $LogoName)) {
            $CustomLogo = Test-AbrLogo -LogoPath (Get-ChildItem -Path $Logo).FullName -IconPath $IconPath -ImagesObj $ImagesObj
        } elseif ($LogoName) {
            $CustomLogo = $LogoName
        } else {
        ...
        if ($SignatureLogo -and (-not $SignatureLogoName )) {
            $CustomSignatureLogo = Test-AbrLogo -LogoPath (Get-ChildItem -Path $SignatureLogo).FullName -IconPath $IconPath -ImagesObj $ImagesObj
    ...
    process {
        ...
        foreach ($OutputFormat in $Format) {
            #Export the Diagram
            if ($Graph) {
                $FilteredGraph = $Graph | Select-String -Pattern '(?s)"?\w+"?\s+\[\s*label="";\s*shape="point";\s*style="invis";\s*\]' -NotMatch | Select-String -Pattern '(?s)"?\w+"?\s+\[\s*label="",\s*shape="point",\s*style="invis",\s*\]' -NotMatch | Select-String -Pattern '(?s)"?\w+"?\s+\[\s*label="",\s*shape=point,\s*style=invis,\s*\]' -NotMatch
