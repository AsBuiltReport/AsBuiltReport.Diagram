function Export-AbrDiagram {
    ...
    .DESCRIPTION
        The Export-AbrDiagram function exports a diagram in PDF, PNG, SVG, or base64 formats using PSgraph.
    ...
    process {
        ...
        try {
            ...
            if ($Format -eq 'svg') {
                ...
                if ($WaterMarkText) {
                    if ($Format -eq 'pdf') {
                        $Document = Add-AbrWatermarkToImage -ImageInput $Document.FullName -WaterMarkText $WaterMarkText -FontColor $WaterMarkColor -FontOpacity $WaterMarkFontOpacity
                    } else {
                        Add-AbrWatermarkToImage -ImageInput $TempOutPut -DestinationPath $DestinationPath -WaterMarkText $WaterMarkText -FontColor $WaterMarkColor -FontOpacity $WaterMarkFontOpacity
                    }
                }
