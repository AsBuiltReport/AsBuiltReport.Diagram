        ...
        if ($Document) {
            ...
            if ($WaterMarkText) {
                if (Get-Command -Name 'Add-WatermarkToSvg' -ErrorAction SilentlyContinue) {
                    $null = Add-WatermarkToSvg -SourceSvgFilePath $Document.FullName -WatermarkText $WaterMarkText -OutputSvgFilePath $Document.FullName -WatermarkTextFontColor $WaterMarkColor -WatermarkTextOpacity $WaterMarkFontOpacity
                } else {
